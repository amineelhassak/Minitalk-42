...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/29 07:53:43 by amel-has          #+#    #+#             */
/*   Updated: 2023/11/29 07:53:47 by amel-has         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_com_mesg(int seg)
{
	if (seg == SIGUSR2)
		ft_putstr_fdd(MSGR, 1);
}
...
int	main(int c, char **v)
{
	...
	if (c != 3 || !ft_isalldigit(v[1]) || v[1][0] == '\0')
	{
	...
	if (ft_printf(PIDC, getpid()) < 0)
		return (ft_putstr_fdd(MSG_ERPPID, 1), -1);
	i = -1;
	while (v[2][++i])
	{
